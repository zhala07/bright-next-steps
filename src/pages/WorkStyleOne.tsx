...
export const WorkStyleOne: React.FC = () => {
  const { work_style, setWorkStyle, setCurrentStep } = useQuizStore();
  const navigate = useNavigate();
  const [selectedValue, setSelectedValue] = useState(work_style.conscientiousness || '');

  const handleNext = () => {
    setWorkStyle('conscientiousness', selectedValue);
    setCurrentStep(5);
    navigate('/quiz/work-style-2');
  };
